...
def RSI(x_real, ncc, period_RSI):
    ...
    for i in range(len(x_real)):
        ...
        if sum(loss[i-period_RSI+1:i+1])!=0:
            RSI.append(1 - 1/( 1 + sum(gain[i - period_RSI + 1 : i + 1])/sum(loss[i-period_RSI + 1 : i + 1]) ) )
        # Если потери равны нулю на периоде
        else:
            RSI.append(1 - 1 / (1 + sum(gain[i - period_RSI + 1 : i + 1]) / 10**-8))
...
def pct_price_change(data, y, x, xname, yname, ncc1, ncc2):
    ...
    pyplot.axhline(y=mean_pct_change(x[yname])[0],linestyle='--',linewidth= 1.2, color ='green', alpha = 1.0,label=ncc1+'_average_change')
    pyplot.axhline(y=mean_pct_change(x[yname])[1],linestyle='--',linewidth= 1.2, color ='green', alpha = 1.0)
    pyplot.axhline(y=mean_pct_change(x[xname])[0],linestyle='--',linewidth= 1.2, color ='y', alpha = 1.0, label=ncc2+'_average_change')
    pyplot.axhline(y=mean_pct_change(x[xname])[1],linestyle='--',linewidth= 1.2, color ='y', alpha = 1.0)
    ...
    print ('Среднее отклонение за интервал ', str(ncc1), ': ',
           round((-mean_pct_change(x[yname])[1]+mean_pct_change(x[yname])[0])*100/2, 3),'%','\n',
       'Среднее отклонение за интервал ', str(ncc2), ': ',
           round((-mean_pct_change(x[xname])[1]+mean_pct_change(x[xname])[0])*100/2, 3),'%' , sep ='')
...
def correlation(Y, x):
    ...
    Y_cor = DataFrame(Y[:len(x)], columns=['Next_BTC_close'])
    X_cor = x.join(Y_cor)
    Corrwith_data = DataFrame.corrwith(X_cor, X_cor['Next_BTC_close'])
...
def test_train_split(x, Y, test_size):
    ...
    print ('В обучающей выборке дней c положительной динамики:', sum(y_train == 1), ', отрицательной:', sum(y_train == 0), sep='')
...
def predict_accuracy(predict,percent,d_ind,y,y_test,cash, cash_start, fl_print, fl_print_trades, x_date, yname, fl_stats, n):
    ...
    if fl_print:
        if n:
            ...
            plt.plot(cash_pointDF, ".", label='Deposit '+n+', pct: '+str(round(percent*100,1)))
...
def data_deposit_plot(predict, data_plot, d_ind,y, y_test, cash, cash_start, best_percent, data, yname, X_train, X_np):
    ...
    predict_accuracy(predict, best_percent, d_ind, y, y_test, cash, cash_start,fl_print = True,
                     fl_print_trades = False, x_date = data['time'],yname=yname, fl_stats=False, n=None)
...
def X_Y_form(full_data, ncc1, ncc2):
    ...
    x_real = DataFrame(full_data)  #сохраняем датафрейм до перевода в процентные изменения
    yname = ncc1 + 'close'
